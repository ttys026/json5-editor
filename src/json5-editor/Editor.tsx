...
type Props = React.ElementType<'div'> & {
  ...
  preClassName?: string;
  virtualList: React.RefObject<FixedSizeList<any>>;
...
export default class Editor extends React.Component<Props, State> {
  ...
  _input: React.RefObject<HTMLTextAreaElement> = React.createRef<HTMLTextAreaElement>();
  _pre: React.RefObject<HTMLPreElement> = React.createRef<HTMLPreElement>();

  componentDidMount() {
    this._recordCurrentState();
    this.props.lineNumberRef.current?.addEventListener('scroll', () => {
      const { scrollTop = 0 } = this.props.lineNumberRef.current || {};
      if (this._input.current) {
        this._input.current.scrollTop = scrollTop;
      }
      this.props.virtualList.current?.scrollTo(scrollTop);
    });
  }
  ...
  _recordChange = (record: Record, overwrite = false) => {
  ...
  _handleKeyDown = (e: any) => {
    ...
    if (e.keyCode === KEYCODE_TAB && !ignoreTabKey && this.state.capture) {
      ...
      const { scrollTop } = this._input.current!;
      // this._pre?.setAttribute('style', `transform: translate(-${scrollLeft}px, ${scrollTop}px)`)
      this._input.current?.scrollTo({ top: scrollTop, left: 0 });
      this.props.virtualList.current?.scrollTo(scrollTop);
      // Ignore selections
    ...
    return (
      <div {...rest} style={{ ...styles.container, ...style }}>
        <textarea
          ...
          onScroll={(e) => {
            const { scrollLeft, scrollTop, clientWidth, scrollWidth } = this._input.current!;
            virtualList.current?.scrollTo(scrollTop);
            this._pre.current!.scrollLeft = scrollLeft;
